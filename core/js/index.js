...
(function(){
  ...
  function refreshAccessStatus() {
    ...
    // TraderLab always first
    traderLabBtn.disabled = !traderLabEligible;
    traderLabStatus.textContent = traderLabEligible ? "🟢 AVAILABLE" : "🔒 LOCKED";

    // CPilot unlocks only if TraderLab completed
    cPilotBtn.disabled = !traderLabCompleted;
    cPilotStatus.textContent = traderLabCompleted ? "🟢 AVAILABLE" : "🔒 LOCKED";

    // Trading Floor unlocks only if CPilot qualified AND tradingFloor approved
    const tfUnlocked = cPilotQualified && tradingFloorApproved;
    tradingFloorBtn.disabled = !tfUnlocked;
    tradingFloorStatus.textContent = tfUnlocked ? "🟢 AVAILABLE" : "⛔ RESTRICTED";
  }
