...
document.addEventListener("DOMContentLoaded", () => {
  ...
  hideAllModules();
  document.getElementById("tradingfloor").style.display = "block";
  ...
  // Simulate trade function (corrected template literal)
  function simulateTrade(asset) {
    ...
    return `Asset: ${asset} | Market Trend: ${trend} | Suggested Action: ${action}`;
  }

  // Event listener for TraderLab buttons
  traderLabPanel.addEventListener("click", e => {
    if (e.target.classList.contains("simulate-btn")) {
      const selectedAsset = e.target.dataset.asset;
      const display = traderLabPanel.querySelector(".trade-result");
      display.textContent = simulateTrade(selectedAsset);
    }
  });
